...
export async function generateLessonPlan(topic: string, depth: string): Promise<LessonPlanData[]> {
  logger.info('🤖 OpenAI generateLessonPlan called with:', { topic, depth })
  ...
  logger.debug('📄 Using depth context:', { context: depthContext[depth as keyof typeof depthContext] })
...
The lesson sequence should feel like a guided journey from "I've never heard of this" to "I understand how to use this in my life/work."`
  ...
  try {
    ...
    logger.debug('⚙️ OpenAI request config:', {
    ...
    logger.info('✅ Lesson plan validation passed:', { lessonsCount: lessons.length })
    return lessons
  } catch (error) {
...
export async function generateLessonContent(
  ...
  logger.info('🤖 OpenAI generateLessonContent called with:', { 
    topic, 
    depth, 
    lessonTitle, 
    duration 
...
Just write the exact words that should be spoken to the listener, as if you are directly teaching them through audio.`
  ...
  logger.debug('📝 Generated content prompt:', { prompt })
  ...
  try {
    ...
    logger.debug('⚙️ OpenAI content request config:', {
      model: requestConfig.model,
      max_completion_tokens: requestConfig.max_completion_tokens,
      messages_count: requestConfig.messages.length
    })
    ...
    logger.debug('💬 Raw OpenAI content response length:', { contentLength: content?.length || 0 })
